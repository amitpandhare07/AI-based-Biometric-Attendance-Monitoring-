...
def extract_embedding_for_image(stream_or_bytes):
    ...
    x, y, w, h = faces[0]
    # Create a mock detection object similar to MediaPipe's format
    class MockDetection:
        def __init__(self, x, y, w, h, img_h, img_w):
            self.location_data = MockLocationData(x, y, w, h, img_h, img_w)
    
    class MockLocationData:
        def __init__(self, x, y, w, h, img_h, img_w):
            self.relative_bounding_box = MockBoundingBox(x, y, w, h, img_h, img_w)
    ...
    detection = MockDetection(x, y, w, h, img.shape[0], img.shape[1])
    emb = crop_face_and_embed(img, detection)
    return emb
